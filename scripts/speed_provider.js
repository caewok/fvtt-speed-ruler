...
export class SpeedProvider {
	...
	getSetting(settingId) {
		try {
			return game.settings.get(MODULE_ID, `speedProviders.${this.id}.setting.${settingId}`)
		}
		catch (e) {
			if (this.settings.some(setting => setting.id === settingId)) {
				throw e
			}
			throw new Error(`MODULE_ID | "${settingId}" is not a registered setting for "${this.id}". If you're the module/system developer, please add it to the return values of your Speed Providers "get settings()" function.`)
		}
	}
